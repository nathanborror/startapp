...
import (
	...
	"github.com/nathanborror/startapp/graphql/errors"
)
...
func (s *Schema) Parse(data []byte) error {
	...
	for i := len(s.Objects) - 1; i >= 0; i-- {
		obj := s.Objects[i]
		if obj.Name == s.EntryPointNames["query"] {
			s.Queries = append(s.Queries, obj.Fields...)
			s.Objects = append(s.Objects[:i], s.Objects[i+1:]...)
		}
		if obj.Name == s.EntryPointNames["mutation"] {
			s.Mutations = append(s.Mutations, obj.Fields...)
			s.Objects = append(s.Objects[:i], s.Objects[i+1:]...)
		}
	}
	return nil
...
func parseSchema(s *Schema, l *common.Lexer) {
	for l.Peek() != scanner.EOF {
		...
		case "scalar":
			name := l.ConsumeIdent()
			scalar := &Scalar{Name: name, Desc: desc}
			s.Types[name] = scalar
